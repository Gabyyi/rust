...
async fn main(spawner: Spawner) {
    ...
    let mut yellow_led = Pwm::new_output_a(peripherals.PWM_SLICE3, peripherals.PIN_6, PwmConfig::default());
    let mut green_led = Pwm::new_output_a(peripherals.PWM_SLICE4, peripherals.PIN_8, PwmConfig::default());

    spawner.spawn(blink_led(yellow_led,333)).unwrap();
    spawner.spawn(blink_led(red_led,250)).unwrap();
    spawner.spawn(blink_led(green_led,200)).unwrap();
