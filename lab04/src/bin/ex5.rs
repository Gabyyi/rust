...
async fn main(_spawner: Spawner) {
    ...
    let mut button1 = Input::new(peripherals.PIN_7, Pull::Up);
    let mut button2 = Input::new(peripherals.PIN_8, Pull::Up);
    ...
    loop {
        match state {
            TrafficLightState::Green => {
                ...
                let button_sequence_future = async {
                    let button1_future = async {
                        while button1.is_high() {
                            Timer::after(Duration::from_millis(10)).await;
                        }
                    };

                    let button2_future = async {
                        while button2.is_high() {
                            Timer::after(Duration::from_millis(10)).await;
                        }
                    };

                    join(button1_future, button2_future).await;
                };

                match select(timer, button_sequence_future).await {
                    Either::First(_) => {
