...
async fn main(spawner: Spawner) {
    ...
    let mut b1 = Input::new(p.PIN_13, Pull::Up);
    let mut b2 = Input::new(p.PIN_14, Pull::Up);
    let mut b3 = Input::new(p.PIN_15, Pull::Up);

    let config = NetConfig::ipv4_static(StaticConfigV4 {
        address: Ipv4Cidr::new(Ipv4Addr::new(69, 69, 69, 70), 24),
    ...
    let pico_ssid = "pico2w";
    let pico_password = "pipico2w";
    ...
    loop {
        ...
        let _ = socket.write(b"Hello from Pico\n").await;

        loop {
            if b1.is_low() {
                info!("Button 1 pressed");
                let _ = socket.write(b"button1:pressed\n").await;
            } else {
                info!("Button 1 not pressed");
            }
            if b2.is_low() {
                info!("Button 2 pressed");
                let _ = socket.write(b"button2:pressed\n").await;
            } else {
                info!("Button 2 not pressed");
            }
            if b3.is_low() {
                info!("Button 3 pressed");
                let _ = socket.write(b"button3:pressed\n").await;
            } else {
